...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function collectFormData() {
        const formData = {};
        const inputs = document.querySelectorAll('#multi-step-form input, #multi-step-form textarea, #multi-step-form select');
    ...
    function submitForm() {
        ...
        // Construct a unique ID for this submission
        const uniqueId = Date.now();

        // Create a structured experience object, mapping form fields to a standard format
        const newExperience = {
            id: uniqueId,
            category: "hackathon",
            companyName: formData['hackathon-name'],
            role: formData['project-title'], // Using project title as a proxy for the 'role'
            author: formData['team-name'] || "N/A", // Using team name as author, or you can get a user's name
            date: formData['hackathon-date'],
            location: formData['location'],
            technicalQuestions: formData['challenges'],
            hrQuestions: formData['best-part'],
            tips: formData['tips'],
            status: "pending", // Set the status to pending for admin review
            upvotes: [],
            downvotes: [],
            comments: [],
            // You can add more fields if needed, like the project category or description
            projectCategory: formData['project-category'],
            projectDescription: formData['project-description'],
            techStack: [...(formData['tech-stack'] || []), formData['other-tech']].filter(Boolean).join(', ') // Combine checkbox and text field data
        };

        // Retrieve existing submissions from localStorage
        const existingSubmissions = JSON.parse(localStorage.getItem('hackathonSubmissions')) || [];
        
        // Add the new submission to the array
        existingSubmissions.push(newExperience);
        
        // Save the updated array back to localStorage
        localStorage.setItem('hackathonSubmissions', JSON.stringify(existingSubmissions));
        
        console.log('Final Form Data:', newExperience);
        ...
        messageBox.innerHTML = `
            <div class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center p-4">
                <div class="bg-white rounded-xl p-8 shadow-2xl max-w-sm text-center">
                    ...
                    <p class="text-gray-600 mb-6">Your hackathon experience has been submitted for review. Thank you for sharing!</p>
    ...
    function updateForm() {
        ...
        navItems.forEach((navItem, index) => {
            ...
            if (circle && label) {
                // Reset classes
                circle.classList.remove('bg-blue-600', 'text-white', 'bg-gray-300', 'text-gray-500');
                label.classList.remove('text-blue-600', 'font-medium', 'text-gray-500');
